...
int LoadCpu1Image(int fd, const char * filename) {
    ...
    if (cpu1_load_ptr == MAP_FAILED) {
        PRINTF_WITH_LOG("mmap cpu1_load_ptr failed, len=0x%x addr=0x%x\n\r", CPU1_CODE_LEN, CPU1_STARTADR);
        return -1;
    } else {
        PRINTF_WITH_LOG("mmap cpu1_load_ptr success, len=0x%x addr=0x%x\n\r", CPU1_CODE_LEN, CPU1_STARTADR);
    }
    ...
    if (CPU1_CODE_LEN <= load_length) {
        PRINTF_WITH_LOG("<%s> file len is larger than %d, file_len=%d\n\r", filename, CPU1_CODE_LEN, load_length);
        munmap(cpu1_load_ptr, CPU1_CODE_LEN);
        return -1;
    }
    ...
    if (NULL == buffer) {
        PRINTF_WITH_LOG("malloc failed, load_length=%d\n\r", load_length);
        munmap(cpu1_load_ptr, CPU1_CODE_LEN);
        return -1;
    }
    return_len = fread(buffer, 1, load_length, core1_code);
    if (return_len != load_length) {
        PRINTF_WITH_LOG("mmap return_len failed, %d != %d\n\r", return_len, load_length);
        free(buffer);
        munmap(cpu1_load_ptr, CPU1_CODE_LEN);
        return -1;
    } else {
        PRINTF_WITH_LOG("success load <%s>, load_length=%d.\n\r", filename, return_len);
    }
    usleep(200*1000);   //等待100ms
    memcpy(cpu1_load_ptr, buffer, load_length);
    usleep(200*1000);   //等待100ms 
    if (memcmp(cpu1_load_ptr, buffer, load_length)) {
        PRINTF_WITH_LOG("memcmp failed, file len=%d\n\r", load_length);
        free(buffer);
        munmap(cpu1_load_ptr, CPU1_CODE_LEN);
        return -1;
    }
...
int bmrun(char *filename) {
    ...
    if (filename != NULL) {
        ...
        if (IfFileExist(szBin)) {
            ...
            int success = 0;
            for (int i=0; i<3;i++)  //最多试三次
            {
                if (LoadCpu1Image(fd, szBin) < 0)
                {
                    PRINTF_WITH_LOG("LoadCpu1Image failed (errno=%d), t=%d\n\r", errno, i);
                }
                else
                {
                    success = 1;
...
uint8 FPGA_CPLD_READ (DWORD addr)
{
	int i;
	uint8 regVal = 0;
	regVal = devmem_readl(FPGA_CPLD_BASE + addr);
...
void FPGA_CPLD_WRITE (DWORD addr,BYTE regVal)
{
	int i;
	devmem_writel(FPGA_CPLD_BASE + addr, regVal);
	for(i = 0; i < 200; i++);
}
