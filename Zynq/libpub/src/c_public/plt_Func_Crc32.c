...
#define SBIT    BIT(31)
#define GEN     (uint32)0x04C11DB7

SECTION_PLT_DATA static uint32 crctab[256];

SECTION_PLT_CODE
static uint32 crc_remainder(int m, uint32_t *r)
{
    uint32 rem = 0;
    int i;
...
uint32 f_crc32(const uint8 * buffer, uint32 size)
{
    ...
    crc = crc32_calc(crc, buffer, size);
...
#define _BUFSIZE     256
    ...
    **那么接收端也同样需要使用该值进行计算*/  
    ...
    fd = E_OPEN(in_file, O_RDONLY, 0);
    if (fd < 0) {  
        PRINTF("%d:open.\n", __LINE__);
        return -1;  
    }  
    
    while ((nread = E_READ(fd, buf, _BUFSIZE)) > 0) {
        crc = crc32_calc(crc, buf, nread);  
    }  
